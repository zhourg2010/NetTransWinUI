...
# 意思就是"投影是薄的，线是硬的"。
DESIGN = [
    0.0, 0.1, 0.1, 0.1, 0.6, 0.5, 0.5, 0.3, 0.9, 0.8,
    1.6, 1.5, 2.1, 1.8, 2.5, 2.8, 3.3, 2.9, 3.6, 3.8,
    4.0, 4.5, 4.9, 5.2, 5.4, 5.8, 6.2, 7.0, 7.1, 7.8,
    8.3, 8.1, 9.3, 9.6, 10.4, 10.8, 11.4, 11.9, 12.9, 13.3,
    14.0, 14.7, 15.1, 16.0, 16.6, 17.5, 18.1, 18.9, 19.7, 20.2,
    100.0,
]
...
# 常数就是用来把这点漂移除掉的。
DESIGN_BG, DESIGN_SEAM = 247.6, 203.0
...
def clean_ramp(px, seam, w, h):
    ...
    for y in range(60, h - 60):
        ...
        # 整段都得是平缓下落。投影每像素掉不到一级，掉得更急的一定是
        # 内容：列表卡片的右缘正好在缝左 16px（卡片内缩 16），白底 255
        # 接到组底 242 是一步 13 级。先前只查最左边那一小段平不平，于是
        # 这批行全被当成"干净"，量出来缝附近凭空多出一道 5 级的坎 —— 那
        # 是卡片的边，不是投影。差点因此把本来正确的 alpha 砍掉一半。
        body = line[:-1]
        if any(abs(body[i + 1] - body[i]) > 2 for i in range(len(body) - 1)):
            continue
        if any(v > bg for v in body):
            continue
